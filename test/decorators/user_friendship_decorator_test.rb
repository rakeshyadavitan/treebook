...
class UserFriendshipDecoratorTest < ActiveRecord::TestCase

	context "#sub_message" do

		setup do
			@friend = create(:user, first_name: 'Ravi')
		end
		...
		context "with a pending user friendship" do
			...
			should "return the correct message" do
				assert_equal "Do you really want to friends with Ravi?",@decorator.sub_message
			end
		...
		context "with an accepted user friendship" do
			...
			should "return the coorect message" do
				assert_equal "You are friends with Ravi.",@decorator.sub_message
			end
